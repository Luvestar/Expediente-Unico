...
class DocumentoController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('proteccion.documentos.index', compact('contribuyentes'));
    }

    public function create($contribuyente_id)
    {
        $contribuyente = Contribuyente::findOrFail($contribuyente_id);
        return view('proteccion.documentos.create', compact('contribuyente'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('documento_final')) {
            $documentoFinalPath = $request->file('documento_final')->store(
                'proteccion/documentos_finales/' . $request->contribuyente_id,
                'public'
            );
        }
        ...
        if ($request->hasFile('archivos')) {
            foreach ($request->file('archivos') as $key => $file) {
                $ruta = $file->store('proteccion/documentos/' . $request->contribuyente_id, 'public');
                $archivosKeys[] = $key;
