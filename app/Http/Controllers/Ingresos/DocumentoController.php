...
class DocumentoController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('ingresos.documentos.index', compact('contribuyentes'));
    }

    public function create($contribuyente_id)
    {
        $contribuyente = Contribuyente::findOrFail($contribuyente_id);
        return view('ingresos.documentos.create', compact('contribuyente'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('documento_final')) {
            $documentoFinalPath = $request->file('documento_final')->store(
                'ingresos/documentos_finales/' . $request->contribuyente_id,
                'public'
            );
        }
        ...
        if ($request->hasFile('archivos')) {
            foreach ($request->file('archivos') as $key => $file) {
                $ruta = $file->store('ingresos/documentos/' . $request->contribuyente_id, 'public');
                $archivosKeys[] = $key;
        ...
        return redirect()->route('ingresos.documentos.cargar', $request->contribuyente_id)
