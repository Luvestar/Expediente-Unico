...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            ...
            {{-- HEADER ESTÁTICO (tarjetas + exportar + título actividad) --}}
            <div class="sticky-header">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <form action="{{ route('industria.historial.exportar.csv') }}" method="GET" class="row g-3">
                            <div class="col-md-3">
            ...
            {{-- TABLA DE ACTIVIDAD CON SCROLL Y HEADERS FIJOS --}}
            ...
            {{-- TÍTULO VIGENCIA DE DOCUMENTOS (estático) --}}
            <div class="card-header bg-success text-white mt-4">
                <div class="d-flex justify-content-between align-items-center">
                    <h4><i class="fas fa-clock"></i> Vigencia de documentos</h4>
                    <div class="btn-group">
                        <a href="{{ route('industria.historial') }}" class="btn btn-sm btn-light {{ !request('estado') ? 'active' : '' }}">Todos</a>
                        <a href="{{ route('industria.historial', ['estado' => 'vigente']) }}" class="btn btn-sm btn-light {{ request('estado') == 'vigente' ? 'active' : '' }}">Vigentes</a>
                        <a href="{{ route('industria.historial', ['estado' => 'proximo']) }}" class="btn btn-sm btn-light {{ request('estado') == 'proximo' ? 'active' : '' }}">Próximos</a>
                        <a href="{{ route('industria.historial', ['estado' => 'vencido']) }}" class="btn btn-sm btn-light {{ request('estado') == 'vencido' ? 'active' : '' }}">Vencidos</a>
                    </div>
                </div>
            </div>
                                            ...
                                            <tr class="border-bottom">
                                                ...
                                                <td width="20%">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="{{ route('industria.documento.detalle', $doc->id) }}" class="btn btn-outline-info"><i class="fas fa-eye"></i> Detalle</a>
                                                        @if($doc->estado == 'vencido')
                                                            ...
                                                            <form action="{{ route('industria.documento.eliminar', $doc->id) }}" method="POST" class="d-inline">
                                                                @csrf @method('DELETE')
                        ...
                        <div class="modal fade" id="renovarModal{{ $doc->id }}" tabindex="-1">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <form action="{{ route('industria.documento.actualizar', $doc->id) }}" method="POST">
                                        @csrf @method('PUT')
...
/* Scroll solo para la tabla - sin scroll horizontal */
.table-scrollable {
    max-height: 400px;
    overflow-y: auto;
    overflow-x: hidden;
